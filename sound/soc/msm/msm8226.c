...
#ifdef CONFIG_KYOCERA_MSND
#define KC_LO_1_SPK_AMP   0x1
#define KC_LO_2_SPK_AMP   0x2
...
static struct wcd9xxx_mbhc_config mbhc_cfg = {
	...
	.gpio_irq_ptt = PTT_SW_GPIO,
	.gpio_level_push_ptt = PTT_SW_LEVEL_PUSH,
	.ptt_irq_state = 0xFF,
	.ptt_poll_state = 0xFF,
	.ptt_poll_cnt = 0,
	.ptt_sw_state = PTT_SW_STATE_IDLE,
#endif
...
#ifdef CONFIG_KYOCERA_MSND
static int kc_msm8226_ext_spk_pamp;
#else
...
#ifdef CONFIG_KYOCERA_MSND
static int kc_ext_spk_amp_gpio_ctrl = -1;
#else
...
#ifdef CONFIG_KYOCERA_MSND
static void kc_msm8226_ext_spk_power_amp_enable(u32 enable)
{
	oem_board_type board_type   = OEM_BOARD_FAIL_TYPE;
	bool           old_amp      = false;

	board_type = OEM_get_board();

	if ( ( board_type == OEM_BOARD_WS0_TYPE )
	  || ( board_type == OEM_BOARD_WS1_1_TYPE )
	  || ( board_type == OEM_BOARD_WS1_2_TYPE ) ) {
		old_amp = true;
	}

	pr_debug("%s: board_type=%d old_amp=%d\n", __func__, board_type, old_amp);

	if (old_amp == true) {
		/* WS1-3まで処理. 削除すること */
		if (enable) {
			pr_debug("%s: Enabled 5V_CTRL(PM8226  GPIO[%d])\n",__func__, vdd_spkr_gpio);
			gpio_direction_output(vdd_spkr_gpio, 1);
			gpio_tlmm_config(SPK_AMP_OUT, GPIO_CFG_ENABLE);
			pr_debug("%s: Delay 10ms\n",__func__);
			usleep_range(10000, 10000);
			pr_debug("%s: Enabled SPK_VDD(MSM8926 GPIO[%d])\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, enable);
			pr_debug("%s: Delay 45ms\n",__func__);
			usleep_range(45000, 45000);
		} else {
			pr_debug("%s: Disabled SPK_VDD(MSM8926 GPIO[%d])\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			gpio_tlmm_config(SPK_AMP_HI_Z, GPIO_CFG_ENABLE);
			pr_debug("%s: Disabled 5V_CTRL(PM8226  GPIO[%d])\n",__func__, vdd_spkr_gpio);
			gpio_direction_output(vdd_spkr_gpio, 0);
		}
	} else {
		/* HW GPIO Hi/Lo間の2usec waitはGPIO制御処理時間で賄えるため、意図的に削除 */
		/* 途中のパルス作る際はGPIO Hi/Lo出力が必要。OFF時は最終的にHi-Z           */
		/* WS2からの処理 */
		if (enable) {
			usleep_range(10000, 10000);
			local_irq_disable();
			pr_debug("%s: Enabled SPK_VDD(MSM8926 GPIO[%d]-S)\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			gpio_tlmm_config(SPK_AMP_OUT, GPIO_CFG_ENABLE);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 0);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 0);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 0);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			pr_debug("%s: Enabled SPK_VDD(MSM8926 GPIO[%d]-E)\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			local_irq_enable();
			usleep_range(45000, 45000);
		} else {
			local_irq_disable();
			pr_debug("%s: Disabled SPK_VDD(MSM8926 GPIO[%d]-S)\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 0);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 0);
			gpio_direction_output(kc_ext_spk_amp_gpio_ctrl, 1);
			...
			pr_debug("%s: Disabled SPK_VDD(MSM8926 GPIO[%d]-E)\n",__func__, kc_ext_spk_amp_gpio_ctrl);
			local_irq_enable();
		}
	}
...
static void kc_msm8226_ext_spk_power_amp_on(u32 spk)
{
	if (gpio_is_valid(kc_ext_spk_amp_gpio_ctrl)) {
		if (spk & (KC_LO_1_SPK_AMP | KC_LO_2_SPK_AMP)) {
			pr_debug("%s:Enable left and right speakers case spk = 0x%x\n",
				__func__, spk);

			kc_msm8226_ext_spk_pamp |= spk;

			if ((kc_msm8226_ext_spk_pamp & KC_LO_1_SPK_AMP) &&
				(kc_msm8226_ext_spk_pamp & KC_LO_2_SPK_AMP))
				if (kc_ext_spk_amp_gpio_ctrl >= 0) {
					pr_debug("%s  enable power", __func__);
					kc_msm8226_ext_spk_power_amp_enable(1);
				}
		} else  {
...
static void kc_msm8226_ext_spk_power_amp_off(u32 spk)
{
	if (gpio_is_valid(kc_ext_spk_amp_gpio_ctrl)) {
		if (spk & (KC_LO_1_SPK_AMP | KC_LO_2_SPK_AMP)) {
			...
			if (!kc_msm8226_ext_spk_pamp) {
				if (kc_ext_spk_amp_gpio_ctrl >= 0) {
					pr_debug("%s  disable power", __func__);
					kc_msm8226_ext_spk_power_amp_enable(0);
				}
				kc_msm8226_ext_spk_pamp = 0;
			}
		 } else  {
...
static int kc_msm8226_ext_spkamp_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	pr_debug("%s()\n", __func__);

	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (!strncmp(w->name, "Lineout_1 spk amp", 18))
			kc_msm8226_ext_spk_power_amp_on(KC_LO_1_SPK_AMP);
		else if (!strncmp(w->name, "Lineout_2 spk amp", 18))
			kc_msm8226_ext_spk_power_amp_on(KC_LO_2_SPK_AMP);
		else {
		...
		if (!strncmp(w->name, "Lineout_1 spk amp", 18))
			kc_msm8226_ext_spk_power_amp_off(KC_LO_1_SPK_AMP);
		else if (!strncmp(w->name, "Lineout_2 spk amp", 18))
			kc_msm8226_ext_spk_power_amp_off(KC_LO_2_SPK_AMP);
		else {
			pr_err("%s() Invalid Speaker Widget = %s\n",
				__func__, w->name);
			return -EINVAL;
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_MIC("Handset Main Mic", NULL),
	SND_SOC_DAPM_MIC("Headset Mic", NULL),
	SND_SOC_DAPM_MIC("Handset Sub Mic", NULL),
#else
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_SPK("Lineout_1 spk amp", kc_msm8226_ext_spkamp_event),
	SND_SOC_DAPM_SPK("Lineout_2 spk amp", kc_msm8226_ext_spkamp_event),
	SND_SOC_DAPM_SPK("Ext Hac", NULL),
#else
...
#ifdef CONFIG_KYOCERA_MSND
	kc_ext_spk_amp_gpio_ctrl = of_get_named_gpio(pdev->dev.of_node,
				"kc,spk-amp-gpio-ctrl", 0);
	if (kc_ext_spk_amp_gpio_ctrl < 0) {
		dev_err(&pdev->dev, "Looking up %s property %d\n", pdev->dev.of_node->full_name, kc_ext_spk_amp_gpio_ctrl);
	} else {
		ret = gpio_request(kc_ext_spk_amp_gpio_ctrl, "SPEAKER_AMP_CTRL");
		if (ret) {
			dev_err(card->dev,
					"%s: Failed to request spkr gpio ctrl %d\n", __func__, kc_ext_spk_amp_gpio_ctrl);
...
#ifdef CONFIG_KYOCERA_MSND
	if (kc_ext_spk_amp_gpio_ctrl >= 0) {
		gpio_free(kc_ext_spk_amp_gpio_ctrl);
		kc_ext_spk_amp_gpio_ctrl = -1;
	}
#else
	if (ext_spk_amp_gpio >= 0) {
