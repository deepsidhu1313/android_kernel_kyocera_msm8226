...
#ifdef CONFIG_KYOCERA_MSND
int kc_fact_lb_mode_set(int val)
{
	kc_fact_lb_mode = val;
...
static int kc_fact_lb_mode_get(void)
{
	return kc_fact_lb_mode;
}
#endif
...
#ifdef CONFIG_KYOCERA_MSND
static const char *const spk_rcv_switch_text[] = {"Off", "On"};
static const struct soc_enum spk_rcv_switch_enum =
		SOC_ENUM_SINGLE_EXT(2, spk_rcv_switch_text);

static const struct snd_kcontrol_new spk1_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT1_SPK", spk_rcv_switch_enum);

static const struct snd_kcontrol_new spk2_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT2_SPK", spk_rcv_switch_enum);

static const struct snd_kcontrol_new rcv1_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT1_RCV", spk_rcv_switch_enum);

static const struct snd_kcontrol_new rcv2_sw =
	SOC_DAPM_ENUM_VIRT("LINEOUT2_RCV", spk_rcv_switch_enum);
#endif
	...
	case SND_SOC_DAPM_PRE_PMU:
#ifdef CONFIG_KYOCERA_MSND
		{
			struct regulator *micbias_regulator;
			int ret;

			micbias_regulator  = tapan_codec_find_regulator(codec,
					"cdc-s4-micbias");

			if (!micbias_regulator) {
				dev_err(codec->dev, "%s: Regulator %s not defined\n",
				__func__, "cdc-s4mic-bias");
			} else {
				ret = regulator_set_voltage(micbias_regulator,
					WCD9XXX_CDC_BUCK_MV_2P15,
					WCD9XXX_CDC_BUCK_MV_2P15);
				ret = regulator_enable(micbias_regulator);
				if (ret) {
					pr_err("%s: MICBIAS Regulator enable failed, index\n",
							__func__);
				} else {
					pr_debug("%s: Enabled MICBIAS regulator, index\n",
						__func__);
				}
			}
...
#ifdef CONFIG_KYOCERA_MSND
		{
			...
			snd_soc_update_bits(codec, micb_ctl_reg, 1 << w->shift,
						1 << w->shift);
#ifdef CONFIG_KYOCERA_MSND
			if(kc_fact_lb_mode_get())
				snd_soc_update_bits(codec, TAPAN_A_MICB_2_CTL, 0x80 ,0x80);
		}
#endif
		break;
	case SND_SOC_DAPM_POST_PMU:
	...
	case SND_SOC_DAPM_POST_PMD:
...
#ifdef CONFIG_KYOCERA_MSND
		{
			...
			snd_soc_update_bits(codec, micb_ctl_reg, 1 << w->shift,
					    0);
#ifdef CONFIG_KYOCERA_MSND
			snd_soc_update_bits(codec, TAPAN_A_MICB_2_CTL, 0x80 ,0);
		}
#endif
...
#ifdef CONFIG_KYOCERA_MSND
		{
			...
			if (!micbias_regulator) {
				dev_err(codec->dev, "%s: Regulator %s not defined\n",
				__func__, "cdc-s4micbias");
			} else {
				ret = regulator_set_voltage(micbias_regulator,
					WCD9XXX_CDC_BUCK_MV_1P8,
					WCD9XXX_CDC_BUCK_MV_2P15);
				ret = regulator_disable(micbias_regulator);
				if (ret) {
					pr_err("%s: MICBIAS Regulator disable failed, index\n",
							__func__);
				} else {
					pr_debug("%s: Disabled MICBIAS regulator, index\n",
						__func__);
				}
			}
		}
...
#ifdef CONFIG_KYOCERA_MSND
	/* RCV path */
	{"LINEOUT1_RCV", "On", "LINEOUT1"},
	{"LINEOUT2_RCV", "On", "LINEOUT2"},
	/* SPK path */
	{"LINEOUT1_SPK", "On", "LINEOUT1"},
	{"LINEOUT2_SPK", "On", "LINEOUT2"},
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	/* LINEOUT SPK */
	SND_SOC_DAPM_VIRT_MUX("LINEOUT1_SPK", SND_SOC_NOPM, 0, 0, &spk1_sw),
	SND_SOC_DAPM_VIRT_MUX("LINEOUT2_SPK", SND_SOC_NOPM, 0, 0, &spk2_sw),
	/* LINEOUT RCV */
	SND_SOC_DAPM_VIRT_MUX("LINEOUT1_RCV", SND_SOC_NOPM, 0, 0, &rcv1_sw),
	SND_SOC_DAPM_VIRT_MUX("LINEOUT2_RCV", SND_SOC_NOPM, 0, 0, &rcv2_sw),
#endif
};
