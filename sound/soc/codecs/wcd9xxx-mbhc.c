...
#ifdef CONFIG_KYOCERA_MSND
static ulong jack_status = 0;
module_param(jack_status, ulong, S_IRUGO );
MODULE_PARM_DESC(jack_status, "jack status");
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	if (mbhc->current_plug == PLUG_TYPE_GND_MIC_SWAP) {
		pr_debug("%s: OMTP (European) headset 0x%x \n", __func__, status);
		if (status == SND_JACK_UNSUPPORTED) {
			status = SND_JACK_HEADPHONE;
			pr_debug("%s: OMTP Headset -> Headphone \n", __func__);
		}
	}

	jack_status = (jack_status & ~mask) | (status & mask);
#endif
...
#ifdef CONFIG_KYOCERA_MSND_PTT
static void wcd9xxx_ptt_sw_set_irq(struct wcd9xxx_mbhc_config *mbhc_cfg, bool enable)
{
	pr_debug("%s enable = %d\n", __func__, enable);

	if (mbhc_cfg->ptt_irq_state == enable)
		return;

	if (enable) {
		enable_irq(gpio_to_irq(mbhc_cfg->gpio_irq_ptt));
		enable_irq_wake(gpio_to_irq(mbhc_cfg->gpio_irq_ptt));
	} else {
		disable_irq_wake(gpio_to_irq(mbhc_cfg->gpio_irq_ptt));
		disable_irq(gpio_to_irq(mbhc_cfg->gpio_irq_ptt));
		mbhc_cfg->ptt_sw_state = PTT_SW_STATE_IDLE;
	}

	mbhc_cfg->ptt_irq_state = enable;
}
#endif
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
...
#ifdef CONFIG_KYOCERA_MSND_PTT
		wcd9xxx_ptt_sw_set_irq(mbhc->mbhc_cfg, 0);
		pr_debug("%s: *** PTT_SW PRESS WAIT CANCEL*** \n", __func__);
		cancel_delayed_work_sync(&mbhc->mbhc_ptt_sw_wait_dwork);
#endif
...
#ifdef CONFIG_KYOCERA_MSND
		if((mbhc->current_plug == PLUG_TYPE_HEADPHONE) || (mbhc->current_plug == PLUG_TYPE_HEADSET) ||
		   (mbhc->current_plug == PLUG_TYPE_GND_MIC_SWAP))
		{
			wcd9xxx_jack_report(mbhc, &mbhc->headset_jack, mbhc->hph_status, WCD9XXX_JACK_MASK);
		}
#else
		...
		if (mbhc->mbhc_cfg->detect_extn_cable) {
			...
			if (mbhc->hph_status && mbhc->hph_status != jack_type) {
				...
				if((mbhc->current_plug == PLUG_TYPE_HEADPHONE) || (mbhc->current_plug == PLUG_TYPE_HEADSET) ||
				   (mbhc->current_plug == PLUG_TYPE_GND_MIC_SWAP))
				{
					wcd9xxx_jack_report(mbhc, &mbhc->headset_jack, 0, WCD9XXX_JACK_MASK);
				}
#else
				wcd9xxx_jack_report(mbhc, &mbhc->headset_jack,
						    0, WCD9XXX_JACK_MASK);
		...
		if (jack_type == SND_JACK_HEADSET)
			wcd9xxx_ptt_sw_set_irq(mbhc->mbhc_cfg, 1);
#endif
	}
	/* Setup insert detect */
...
#ifdef CONFIG_KYOCERA_MSND_PTT
static void wcd9xxx_ptt_sw_lpress_fn(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct wcd9xxx_mbhc *mbhc;
	int val;
	...
	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd9xxx_mbhc, mbhc_ptt_sw_dwork);

	val = gpio_get_value_cansleep(PTT_SW_GPIO);

	if (mbhc->mbhc_cfg->ptt_poll_state == val)
		mbhc->mbhc_cfg->ptt_poll_cnt++;
	else
		mbhc->mbhc_cfg->ptt_poll_cnt = 1;

	pr_debug("PTT_SW_GPIO = %d poll_state = %d poll_cnt = %d\n", val, mbhc->mbhc_cfg->ptt_poll_state, mbhc->mbhc_cfg->ptt_poll_cnt);

	mbhc->mbhc_cfg->ptt_poll_state = val;

	if (val == mbhc->mbhc_cfg->gpio_level_push_ptt) {
		if (mbhc->mbhc_cfg->ptt_poll_cnt >= PTT_SW_POLLING_PUSH_CNT) {
			mbhc->buttons_pressed |= SND_JACK_BTN_0;
			report = 1;
			if ( mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_RELEASE_WAIT ) {
				pr_debug("%s: *** PTT_SW PRESS WAIT CANCEL*** \n", __func__);
				cancel_delayed_work_sync(&mbhc->mbhc_ptt_sw_wait_dwork);
				wcd9xxx_jack_report(mbhc, &mbhc->button_jack, 0, SND_JACK_BTN_0);
				mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_PRESS_WAIT;
			} else {
				mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_PRESS_WAIT;
			}
		}
	} else {
		if (mbhc->mbhc_cfg->ptt_poll_cnt >= PTT_SW_POLLING_RELEASE_CNT) {
			mbhc->buttons_pressed &= ~SND_JACK_BTN_0;
			report = 1;
			if (mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_PRESS_WAIT) {
				pr_debug("%s: *** PTT_SW PRESS WAIT CANCEL*** \n", __func__);
				cancel_delayed_work_sync(&mbhc->mbhc_ptt_sw_wait_dwork);
				mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_IDLE;
			} else if ( mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_PRESS_KEEP ) {
				mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_RELEASE_WAIT;
			} else {
				mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_IDLE;
			}
		}
	...
	if (report) {
		if (mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_PRESS_WAIT) {
			pr_debug("*** PTT_SW PRESS WAIT (%d)***\n", PTT_SW_POLLING_PRESS_TIME);
			schedule_delayed_work(&mbhc->mbhc_ptt_sw_wait_dwork, msecs_to_jiffies(PTT_SW_POLLING_PRESS_TIME));
			pr_debug("%s: Reporting ptt button press event1 (0x%x)\n", __func__, SND_JACK_BTN_0);
			wcd9xxx_jack_report(mbhc, &mbhc->button_jack, SND_JACK_BTN_0, SND_JACK_BTN_0);
		} else if (mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_RELEASE_WAIT) {
			pr_debug("*** PTT_SW PRESS WAIT (%d)***\n", PTT_SW_POLLING_RELEASE_TIME);
			schedule_delayed_work(&mbhc->mbhc_ptt_sw_wait_dwork, msecs_to_jiffies(PTT_SW_POLLING_RELEASE_TIME));
			pr_debug("%s: Reporting ptt button release event1 (0x%x)\n", __func__, SND_JACK_BTN_0);
			wcd9xxx_jack_report(mbhc, &mbhc->button_jack, SND_JACK_BTN_0, SND_JACK_BTN_0);
		} else {
			pr_debug("%s: Reporting ptt button press/release event (0x%x)\n", __func__, mbhc->buttons_pressed);
			wcd9xxx_jack_report(mbhc, &mbhc->button_jack, mbhc->buttons_pressed, SND_JACK_BTN_0);
		}
		mbhc->mbhc_cfg->ptt_poll_cnt = 0;
	} else {
		pr_debug("*** PTT_SW POLLING GPIO (%d) COUNT (%d) ***\n", val, mbhc->mbhc_cfg->ptt_poll_cnt);
		schedule_delayed_work(&mbhc->mbhc_ptt_sw_dwork, msecs_to_jiffies(PTT_SW_POLLING_ONE_TIME));
	}

	pr_debug("%s: leave(sw_stat:%d)\n", __func__, mbhc->mbhc_cfg->ptt_sw_state);
}

static void wcd9xxx_ptt_sw_wait_fn(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct wcd9xxx_mbhc *mbhc;
	...
	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd9xxx_mbhc, mbhc_ptt_sw_wait_dwork);

	if  (mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_PRESS_WAIT) {
		pr_debug("%s: Reporting ptt button press event2 (0x%x)\n", __func__, 0);
		wcd9xxx_jack_report(mbhc, &mbhc->button_jack, 0, SND_JACK_BTN_0);
		mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_PRESS_KEEP;
	} else if (mbhc->mbhc_cfg->ptt_sw_state == PTT_SW_STATE_RELEASE_WAIT) {
		pr_debug("%s: Reporting ptt button release event2 (0x%x)\n", __func__, 0);
		wcd9xxx_jack_report(mbhc, &mbhc->button_jack, 0, SND_JACK_BTN_0);
		mbhc->mbhc_cfg->ptt_sw_state = PTT_SW_STATE_IDLE;
    }
...
#ifdef CONFIG_KYOCERA_MSND_PTT
static irqreturn_t wcd9xxx_ptt_sw_irq(int irq, void *data)
{
	struct wcd9xxx_mbhc *mbhc = data;

	pr_debug("%s: enter\n", __func__);

	if (gpio_to_irq(mbhc->mbhc_cfg->gpio_irq_ptt) != irq)
	{
		return IRQ_NONE;
	}

	cancel_delayed_work_sync(&mbhc->mbhc_ptt_sw_dwork);
	pr_debug("%s: *** PTT_SW PRESS WAIT CANCEL*** \n", __func__);
	cancel_delayed_work_sync(&mbhc->mbhc_ptt_sw_wait_dwork);
	schedule_delayed_work(&mbhc->mbhc_ptt_sw_dwork, msecs_to_jiffies(1));
	pr_debug("%s: leave\n", __func__);
...
static irqreturn_t wcd9xxx_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD9XXX_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
#ifdef CONFIG_KYOCERA_MSND_PTT
			pr_debug("%s: Wait 20msec for 4pole PTT Headset\n", __func__);
...
#ifdef CONFIG_KYOCERA_MSND_PTT
	ret = request_threaded_irq(gpio_to_irq(mbhc->mbhc_cfg->gpio_irq_ptt), NULL,
								wcd9xxx_ptt_sw_irq,
								(IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING),
								"ptt sw detect", mbhc);
	if (ret) {
		pr_err("%s: Failed to request gpio irq ptt %d\n", __func__, gpio_to_irq(mbhc->mbhc_cfg->gpio_irq_ptt));
	} else {
		wcd9xxx_ptt_sw_set_irq(mbhc->mbhc_cfg, 0);
	}
#endif
...
#ifdef CONFIG_KYOCERA_MSND_PTT
	free_irq(gpio_to_irq(mbhc->mbhc_cfg->gpio_irq_ptt), mbhc);
#endif
}
...
#ifdef CONFIG_KYOCERA_MSND_PTT
		printk("INIT_DELAYED_WORK(PTT_SW) call\n");
		INIT_DELAYED_WORK(&mbhc->mbhc_ptt_sw_dwork, wcd9xxx_ptt_sw_lpress_fn);
		printk("INIT_DELAYED_WORK(PTT_SW_WAIT) call\n");
		INIT_DELAYED_WORK(&mbhc->mbhc_ptt_sw_wait_dwork, wcd9xxx_ptt_sw_wait_fn);
#endif
	}
