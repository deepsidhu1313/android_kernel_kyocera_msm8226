...
extern void kernel_disp_raw_bitmap_SR(int, int, int, int, const unsigned char *, char *, int);
#endif
/*@OEMDISPLAY@_E*/
...
/*@OEMDISPLAY@_S*/
#ifdef CONFIG_DISP_EXT_KSPLASH
	kernel_disp_raw_bitmap_SR( 0, 0, fbi->var.xres, fbi->var.yres, splash_bgr888_image, sinfo->splash_buffer, 0);
#else
...
int mdss_mdp_splash_init(struct msm_fb_data_type *mfd)
{
	...
	if (oem_is_off_charge() || oem_is_recoverymode() || oem_is_fotamode()) {
		rc = -EINVAL;
		mfd->splash_info.splash_logo_enabled = 0;
		goto end;
	}
