...
static int pp_copy_pa_six_zone_lut(struct mdp_pa_v2_cfg_data *pa_v2_config,
				u32 disp_num, u32 copy_from_kernel);
...
static int pp_read_pa_v2_regs(char __iomem *addr,
				struct mdp_pa_v2_data *pa_v2_config,
				u32 disp_num, u32 copy_to_kernel);
				...
				u32 disp_num, u32 copy_to_kernel)
{
	...
	if (pa_v2_config->flags & MDP_PP_PA_SIX_ZONE_ENABLE) {
		...
		if (copy_to_kernel) {
			memcpy(pa_v2_config->six_zone_curve_p0,
				&mdss_pp_res->six_zone_lut_curve_p0[disp_num][0],
				pa_v2_config->six_zone_len * sizeof(u32));
			memcpy(pa_v2_config->six_zone_curve_p1,
				&mdss_pp_res->six_zone_lut_curve_p1[disp_num][0],
				pa_v2_config->six_zone_len * sizeof(u32));
		} else {
				...
				u32 disp_num, u32 copy_from_kernel)
{
	...
	if (copy_from_kernel) {
		memcpy(&mdss_pp_res->six_zone_lut_curve_p0[disp_num][0],
			pa_v2_config->pa_v2_data.six_zone_curve_p0,
			pa_v2_config->pa_v2_data.six_zone_len * sizeof(u32));
		memcpy(&mdss_pp_res->six_zone_lut_curve_p1[disp_num][0],
					...
					u32 *copyback, u32 copy_from_kernel)
{
	...
	if (config->ops & MDP_PP_OPS_READ) {
		...
		if (copy_from_kernel) {
			memcpy(config->c0_c1_data, local_cfg.c0_c1_data,
				config->len * sizeof(u32));
			memcpy(config->c2_data, local_cfg.c2_data,
				config->len * sizeof(u32));
		} else {
				...
				u32 *copyback, u32 copy_from_kernel)
{
	...
	if (config->flags & MDP_PP_OPS_READ) {
		...
		if (copy_from_kernel) {
			memcpy(config->r_data,
				&mdss_pp_res->gc_lut_r[disp_num][0],
				tbl_size);
			memcpy(config->g_data,
				&mdss_pp_res->gc_lut_g[disp_num][0],
				tbl_size);
			memcpy(config->b_data,
				&mdss_pp_res->gc_lut_b[disp_num][0],
				tbl_size);
		} else {
		...
		*copyback = 1;
		...
		if (copy_from_kernel) {
			memcpy(&mdss_pp_res->gc_lut_r[disp_num][0],
				config->r_data, r_size);
			memcpy(&mdss_pp_res->gc_lut_g[disp_num][0],
				config->g_data, g_size);
			memcpy(&mdss_pp_res->gc_lut_b[disp_num][0],
