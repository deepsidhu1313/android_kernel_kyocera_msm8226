...
unsigned long g_timeout_frame_id = 0;
extern struct workqueue_struct *frame_wq;
extern struct work_struct cam_frame_mon_work;
static int stream_exe;
struct vfe_device *g_vfe_dev = NULL;
static int stream_condition;
DECLARE_WAIT_QUEUE_HEAD( wait_q );
static DEFINE_SPINLOCK(vfe_timeout_lock);
...
void msm_frame_monitoring(struct work_struct *work){
	struct msm_isp_event_data error_timeout_event;
	int rc;
	unsigned long flags;

	while(stream_exe){
		stream_condition = 0;
		rc = wait_event_interruptible_timeout(wait_q, stream_condition, msecs_to_jiffies(8000));
		if(!rc && stream_exe){
			spin_lock_irqsave(&vfe_timeout_lock, flags);
			stream_exe = 0;
			error_timeout_event.frame_id = g_timeout_frame_id;
			if(g_vfe_dev){
				msm_isp_send_event(g_vfe_dev, ISP_EVENT_TIMEOUT_ERROR, &error_timeout_event);
			}
			spin_unlock_irqrestore(&vfe_timeout_lock, flags);
		}
	}
	return;
}
			...
			enum msm_isp_camif_update_state camif_update)
{
	...
	stream_exe = 1;
	if (frame_wq)
		queue_work(frame_wq, &cam_frame_mon_work);
