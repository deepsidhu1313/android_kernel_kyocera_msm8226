...
static bool oem_fake_done_charging;
				...
				int batt_temp)
{
	...
	} else if (chip->done_charging || oem_fake_done_charging) {
		oem_fake_done_charging = false;
...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->calculated_soc < 0) {
		pr_err("error calculated_soc=%d. return %d\n", chip->calculated_soc, chip->last_soc);
	...
	soc = oem_bms_correct_soc(chip->last_soc, chip->calculated_soc, chip->prev_last_good_ocv_raw);
	pr_debug("Reported SOC = %d last_soc = %d\n", soc, chip->last_soc);
...
#define OEM_1_SECOND_TO_10_MINUTES	(10 * 60)
void oem_bms_reset_cc(void)
{
	u8 bat_fet_status;
	struct timespec ts;
	...
	if (!is_initialized) {
		...
		is_initialized = true;
		pr_info("initialized tv_sec:%d\n", (int)last_ts.tv_sec);
	}

	if (get_battery_status(the_chip) == POWER_SUPPLY_STATUS_FULL) {
		getnstimeofday(&ts);
		elapsed_time = ts.tv_sec - last_ts.tv_sec;
		if (OEM_1_SECOND_TO_10_MINUTES <= elapsed_time) {
			bat_fet_status = oem_chg_get_bat_fet_status();
			if(!bat_fet_status) {
				oem_fake_done_charging = true;
				pr_debug("OEM BMS reset_cc \n");
			}
			getnstimeofday(&last_ts);
			pr_info("elapsed_time:%dsec\n", elapsed_time);
		}
	}
	else {
		oem_fake_done_charging = false;
