...
#define HKADC_DEFAULT_TEMP      25
#define HKADC_DEFAULT_TEMPx10   250
...
#ifndef OEM_SUBSTRATE_THERM_DISABLE
static int substrate_therm_work[HKADC_ELEMENT_COUNT] = {0};
#endif
static int usb_therm_work[HKADC_ELEMENT_COUNT]={0};
...
#endif
static unsigned int index_usb_therm_work = 0;
	...
	enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_OEM_USB_THERM:
		idx = OEM_POWER_SUPPLY_PROP_IDX_USB_THERM;
		break;
			...
			int *hkadc_work)
{
	...
	pr_info("init hkadc ch=%d value=%d \n",
...
static void oem_hkadc_monitor(struct work_struct *work)
{
	...
	if (req_wakeup_monitor) {
...
#endif
		oem_hkadc_get_wakeup_value(POWER_SUPPLY_PROP_OEM_USB_THERM, P_MUX7_1_1,
			usb_therm_work, &index_usb_therm_work, &usb_therm);

		therm_monitor_freq = 0;

		req_wakeup_monitor = false;
		req_ps_changed_batt |= true;
		pr_info("wakeup tv_sec=%d diff:%d vbat:%d temp:%d pa:%d cam:%d sub:%d usb:%d\n",
			(int)ts.tv_sec, (int)(ts.tv_sec - last_ts.tv_sec),
			vbat, temp, pa_therm, camera_therm, substrate_therm, usb_therm);
		last_ts = ts;
		...
		if (therm_monitor_freq >= HKADC_THERM_MONITOR_FREQ) {
			...
			oem_hkadc_get_value(POWER_SUPPLY_PROP_OEM_SUBSTRATE_THERM, P_MUX7_1_1,
				substrate_therm_work, &index_substrate_therm_work, &substrate_therm);
#endif
			oem_hkadc_get_value(POWER_SUPPLY_PROP_OEM_USB_THERM, P_MUX7_1_1,
				usb_therm_work, &index_usb_therm_work, &usb_therm);
			pr_debug("vbat:%d temp:%d pa:%d cam:%d sub:%d usb:%d\n",
				vbat, temp, pa_therm, camera_therm, substrate_therm, usb_therm);
			req_ps_changed_batt |= true;
		}
	}
	...
	if (usb_therm >= USB_THERM_WARNING_TEMP) {
		wake_lock(&oem_hkadc_wake_lock);
		pr_info("wake_lock usb_therm:%d\n", usb_therm);
	} else if (wake_lock_active(&oem_hkadc_wake_lock)) {
...
void oem_hkadc_init(void *chip)
{
	...
	oem_hkadc_master_data[1] = HKADC_DEFAULT_TEMPx10;
	...
	oem_hkadc_master_data[5] = HKADC_DEFAULT_TEMP;  
...
#endif
	oem_hkadc_init_value(POWER_SUPPLY_PROP_OEM_USB_THERM, P_MUX7_1_1,
		usb_therm_work);
