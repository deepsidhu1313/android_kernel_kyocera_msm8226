...
#ifdef CONFIG_OEM_CHG_DET
#include <linux/oem-chg_api.h>
#include <oem-chg_det.h>
#endif


#ifdef CONFIG_OEM_CHG_PARAM
...
#include <oem-bms.h>
#ifdef CONFIG_OEM_OFF_CHG_CHECK
#include <linux/reboot.h>
#include <mach/board.h>
#define OEM_OFF_CHG_CHECK_TIMEOUT_MS	8000
#endif
...
#ifdef CONFIG_OEM_OFF_CHG_CHECK
	bool			oem_off_chg_check_flag;
	struct delayed_work	oem_off_chg_check_work;
#endif
};
...
#ifdef CONFIG_OEM_OFF_CHG_CHECK
static void oem_off_chg_check_timeout(struct work_struct *work);
#endif

static struct qpnp_chg_chip *the_chip;
static bool oem_charging_disabled = false;

#define INITIAL_DELTA_MV	(-60)
static unsigned int oem_recharge_delta_mv = 150;  
...
{
	u8 temp;
	static int current_vbatdet_mv = 0;
	...
	if (current_vbatdet_mv != vbatdet_mv) {
		pr_info("vbatdet_mv old=%d new=%d\n", current_vbatdet_mv, vbatdet_mv);
		current_vbatdet_mv = vbatdet_mv;
	}
...
static bool oem_chg_vbatdet_change_flg = false;

static void
qpnp_chg_set_appropriate_vbatdet(struct qpnp_chg_chip *chip)
{

	if ( (chip->chg_done) || (oem_chg_vbatdet_change_flg == true) ) {
		chip->resume_delta_mv = oem_recharge_delta_mv;
	} else {
		chip->resume_delta_mv = INITIAL_DELTA_MV;
	}


	if (oem_charging_disabled) {
		qpnp_chg_vbatdet_set(chip, QPNP_CHG_VBATDET_MIN_MV);
	} else
	...
	{
		qpnp_chg_vbatdet_set(chip, 4100);   
		pr_info("qpnp_chg_vbatdet_set convert %d to %d\n",
...
#ifdef CONFIG_OEM_OFF_CHG_CHECK
			if (chip->oem_off_chg_check_flag &&
				oem_is_off_charge()) {
				machine_power_off();
			}
#endif
		...
		if (fastchg_on) {
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				...
				if (ret.intval == POWER_SUPPLY_TYPE_USB_DCP) {
					...
					oem_chg_h_chg_on_ctrl(1);
				}
			}
		} else {
			oem_chg_h_chg_on_ctrl(0);
		}
...
switch_usb_to_host_mode(struct qpnp_chg_chip *chip)
{
	...
	oem_chg_set_pma_en(1);
	oem_chg_set_usb_path_en(1);
	pr_info("called\n");
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	POWER_SUPPLY_PROP_OEM_VOLT_STATUS,

	POWER_SUPPLY_PROP_OEM_TEMP_STATUS,
	POWER_SUPPLY_PROP_OEM_USB_ALERT,
};
...
#define USB_OVERVOLUTAGE_BIT	0xc0
...
get_prop_batt_health(struct qpnp_chg_chip *chip)
{
	...
	rc = qpnp_chg_read(chip, &usbin_valid_rt_sts,
				 chip->usb_chgpth_base + CHGR_STATUS , 1);
	if (!rc) {
		if ( 0x40 == (usbin_valid_rt_sts & USB_OVERVOLUTAGE_BIT) ) {
	...
	if (OEM_ST_VOLT_HIGH == oem_chg_get_current_volt_status()) {
...
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	...
	if (OEM_ST_TEMP_LIMIT == oem_chg_get_current_temp_status()) {
		return POWER_SUPPLY_STATUS_CHARGING;
	}
				       ...
				       union power_supply_propval *val)
{
...
#ifdef CONFIG_OEM_CHG_DET
	if (-EINVAL != oem_chg_pm_batt_power_get_property(psp, &(val->intval))) {
		return 0;
	}
#endif
...
#ifdef CONFIG_OEM_HKADC
	if (-EINVAL != oem_hkadc_pm_batt_power_get_property(psp, &(val->intval))) {
		return 0;
	}
#endif
	...
	case POWER_SUPPLY_PROP_OEM_VOLT_STATUS:
		val->intval = oem_chg_get_current_volt_status();
		break;


	case POWER_SUPPLY_PROP_OEM_TEMP_STATUS:
		val->intval = oem_chg_get_current_temp_status();
		break;

	case POWER_SUPPLY_PROP_OEM_USB_ALERT:
		val->intval = oem_chg_get_usb_alert_status();
		break;
	default:
		return -EINVAL;
	}
...
#define VBATDET_MAX_ERR_COOL_MV	-50 
#define VBATDET_MAX_ERR_WARM_MV	 50 
static void
qpnp_eoc_work(struct work_struct *work)
{
	...
	int oem_chg_vbatdet_max_err_mv;
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		if (chip->bat_is_cool)
			oem_chg_vbatdet_max_err_mv = VBATDET_MAX_ERR_COOL_MV;
		else if (chip->bat_is_warm)
			oem_chg_vbatdet_max_err_mv = VBATDET_MAX_ERR_WARM_MV;
		else
			oem_chg_vbatdet_max_err_mv = VBATDET_MAX_ERR_MV;

		vbat_lower_than_vbatdet = !(chg_sts & VBAT_DET_LOW_IRQ);
		if (vbat_lower_than_vbatdet && vbat_mv <
				(chip->max_voltage_mv - oem_recharge_delta_mv
				 - oem_chg_vbatdet_max_err_mv)) {
			vbat_low_count++;
			...
			if (vbat_low_count >= CONSECUTIVE_COUNT) {
				...
				oem_chg_vbatdet_change_flg = true;
				qpnp_chg_set_appropriate_vbatdet(chip);
				oem_chg_vbatdet_change_flg = false;
		...
		if (!(buck_sts & VDD_LOOP_IRQ)) {
			...
			if (count == CONSECUTIVE_COUNT) {
				if (oem_chg_get_current_capacity() < FULL_CHG_CHECK_SOC) {
					count = 0;
					oem_bms_set_catch_up_full(1);
					pr_info("detected a full charge, but soc(%d) is less than %d\n",
						oem_chg_get_current_capacity(), FULL_CHG_CHECK_SOC);
					goto check_again_later;
				}
...
qpnp_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (chip->bat_is_cool ^ bat_cool || chip->bat_is_warm ^ bat_warm) {
		pr_info("bat_is_warm old:%d new:%d bat_is_cool old:%d new:%d\n",
			chip->bat_is_warm, bat_warm, chip->bat_is_cool, bat_cool);
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (1 == oem_param_share.test_mode_chg_disable) {
			pr_info("CHARGING_ENABLED don't change, because usb charging disable\n");
			return 0;
		}
...
qpnp_charger_read_dt_props(struct qpnp_chg_chip *chip)
{
	...
	if (!rc) {
		oem_recharge_delta_mv = chip->resume_delta_mv;
		chip->resume_delta_mv = INITIAL_DELTA_MV;
		pr_info("set resume_delta_mv=%d oem_recharge_delta_mv=%d\n",
			chip->resume_delta_mv, oem_recharge_delta_mv);
	} else {
		pr_err("resume_delta_mv read fail:%d\n", rc);
	}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	if (1 == oem_param_share.test_mode_chg_disable) {
		chip->charging_disabled = true;
		pr_info("charging disabled in test mode\n");
	}
...
#ifdef CONFIG_OEM_OFF_CHG_CHECK
	else if (oem_is_off_charge()) {
		machine_power_off();
	}
	chip->oem_off_chg_check_flag = true;
	INIT_DELAYED_WORK(&chip->oem_off_chg_check_work, oem_off_chg_check_timeout);
	schedule_delayed_work(&chip->oem_off_chg_check_work,
		msecs_to_jiffies(OEM_OFF_CHG_CHECK_TIMEOUT_MS));
#endif
	...
	the_chip = chip;
	oem_chg_smem_init();
	oem_chg_control_init();
	oem_hkadc_init(chip);
	oem_chg_det_init(chip);
	oem_batt_led_init();
...
void oem_chg_ps_changed_batt(void)
{
	power_supply_changed(&the_chip->batt_psy);
}
EXPORT_SYMBOL(oem_chg_ps_changed_batt);


int oem_chg_batt_fet_ctrl(int enable)
{

	oem_charging_disabled = !enable;
	qpnp_chg_set_appropriate_vbatdet(the_chip);

	return qpnp_chg_charge_en(the_chip, enable);
}
EXPORT_SYMBOL(oem_chg_batt_fet_ctrl);

int oem_chg_buck_ctrl(int disable)
{
	return qpnp_chg_force_run_on_batt(the_chip, disable);
}
EXPORT_SYMBOL(oem_chg_buck_ctrl);
...
#define OEM_BAT_FET_ON	BIT(7)
u8 oem_chg_get_bat_fet_status(void)
{
	int rc;
	u8 bat_fet_status;
	...
	rc = qpnp_chg_read(the_chip, &bat_fet_status,
			the_chip->bat_if_base + BAT_IF_BAT_FET_STATUS, 1);
	if(rc) {
		return 0xFF;
	}
	else {
		return (bat_fet_status & OEM_BAT_FET_ON);
	}
}
...
{
	struct qpnp_chg_chip *chip = container_of(work,
				struct qpnp_chg_chip, oem_off_chg_check_work.work);

	chip->oem_off_chg_check_flag = false;
	pr_info("call\n");
}
#endif
