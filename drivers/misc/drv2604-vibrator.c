...
/*--------------------------------------------*/
void drv2604_write_register_exp(unsigned char reg, unsigned char data)
{
	unsigned char w_buf[2];
	w_buf[0] = (u8)(reg);
	w_buf[1] = (u8)(data);
	drv2604_i2c_write_data ( drv2604_data.drv2604_i2c_client, w_buf, sizeof(w_buf) );
}
EXPORT_SYMBOL(drv2604_write_register_exp);
/*--------------------------------------------*/
...
//
//
//	VIB_SET_REG(0x18, 0x07);
//	VIB_SET_REG(0x19, 0x7c);
//	VIB_SET_REG(0x1a, 0xb5);
//	VIB_SET_REG(0x1b, 0x13);
...
//
	/* Change value to write drv2604 via spi. */
	VIB_SET_REG(0x00, 0xA0);
	VIB_SET_REG(0x01, 0x00);
	VIB_SET_REG(0x02, 0x00);
	VIB_SET_REG(0x03, 0x00);
	VIB_SET_REG(0x04, 0x01);
	VIB_SET_REG(0x05, 0x00);
	VIB_SET_REG(0x06, 0x00);
	VIB_SET_REG(0x07, 0x00);
	VIB_SET_REG(0x08, 0x00);
	VIB_SET_REG(0x09, 0x00);
	VIB_SET_REG(0x0A, 0x00);
	VIB_SET_REG(0x0B, 0x00);
	VIB_SET_REG(0x0C, 0x00);
	VIB_SET_REG(0x0D, 0x00);
	VIB_SET_REG(0x0E, 0x00);
	VIB_SET_REG(0x0F, 0x00);
	VIB_SET_REG(0x10, 0x00);
	VIB_SET_REG(0x11, 0x05);
	VIB_SET_REG(0x12, 0x19);
	VIB_SET_REG(0x13, 0x64);
	VIB_SET_REG(0x14, 0x19);
	VIB_SET_REG(0x15, 0xFF);
	VIB_SET_REG(0x16, 0x4E);
	VIB_SET_REG(0x17, 0x8C);
	VIB_SET_REG(0x18, 0x08);
	VIB_SET_REG(0x19, 0x88);
	VIB_SET_REG(0x1A, 0xE5);
	VIB_SET_REG(0x1B, 0x93);
	VIB_SET_REG(0x1C, 0xF5);
	VIB_SET_REG(0x1D, 0x84);
	VIB_SET_REG(0x1E, 0x30);
	VIB_SET_REG(0x1F, 0x80);
	VIB_SET_REG(0x20, 0x33);
...
struct gpio_vib_priv {
	int num;
	struct gpio_vib_data vib[];
};
#define GPIO_DEFSTATE_OFF		0
#define GPIO_DEFSTATE_ON		1
#define GPIO_DEFSTATE_KEEP		2

static inline int sizeof_gpio_vib_priv(int num)
{
	return sizeof(struct gpio_vib_priv) + (sizeof(struct gpio_vib_data) * num);
}

static int create_gpio_vib(
	struct gpio_vib_data *template,
	struct gpio_vib_data *dat,
	struct device *parent)
{
	...
	/* skip vibs that aren't available */
	if (!gpio_is_valid(template->gpio)) {
		dev_err(parent, "Skipping unavailable VIB gpio=%d(%s)\n",
	...
	if (ret < 0) {
		dev_err(parent, "devm_gpio_request failed. gpio=%d(%s)\n",
			template->gpio,
			template->name);
		return ret;
	}
	...
	if (template->default_state == GPIO_DEFSTATE_KEEP) {
		...
		state = (template->default_state == GPIO_DEFSTATE_ON);
	}

	value = template->active_low ^ state;
	ret = gpio_direction_output(template->gpio, value);
	if (ret < 0) {
		dev_err(parent, "gpio_direction_output failed. gpio=%d(%s) state=%d value=%d\n",
			template->gpio,
			template->name,
			state,
			value);
	...
	*dat = *template;
	...
	if (0 == strcmp(dat->name, "vib_intrig")) {
		g_gpio_vib_intrig = dat->gpio;
	}
	else if (0 == strcmp(dat->name, "vib_en")) {
		g_gpio_vib_en = dat->gpio;
	}

	dev_info(parent, "gpio=%d(%s) state=%d value=%d can_sleep=%d active_low=%d\n",
		dat->gpio,
		dat->name,
		state,
		value,
...
static struct gpio_vib_priv *gpio_vib_create_of(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node, *child;
	struct gpio_vib_priv *priv;
	int count;
	int size;

	/* count VIBs in this device, so we know how much to allocate */
	...
	for_each_child_of_node(np, child) {
		if (of_get_gpio(child, 0) == -EPROBE_DEFER) {
			dev_err(&pdev->dev, "of_get_gpio(%p) error.\n", child);
			return ERR_PTR(-EPROBE_DEFER);
		}
	}

	size = sizeof_gpio_vib_priv(count);
	debugk("count=%d size=%d\n", count, size);
	...
	for_each_child_of_node(np, child) {
		...
		def_state = of_get_property(child, "default-state", NULL);

		if (def_state) {
			if (!strcmp(def_state, "keep"))
				vib.default_state = GPIO_DEFSTATE_KEEP;
			else if (!strcmp(def_state, "on"))
				vib.default_state = GPIO_DEFSTATE_ON;
			else
				vib.default_state = GPIO_DEFSTATE_OFF;
		}
		else {
			def_state = "";
		}

		debugk(">>create_gpio_vib %d/%d gpio=%d(%s) default_state=%d(%s)\n",
			priv->num, count, vib.gpio, vib.name, vib.default_state, def_state);
		create_gpio_vib(&vib, &priv->vib[priv->num++], &pdev->dev);
	}

	return priv;
}

static int gpio_vib_probe(struct platform_device *pdev)
{
	struct gpio_vib_priv *priv;

	debugk(">>gpio_vib_create_of(%p)\n", pdev);
	priv = gpio_vib_create_of(pdev);

	debugk(">>platform_set_drvdata(%p, %p)\n", pdev, priv);
	platform_set_drvdata(pdev, priv);

	return 0;
}
static int gpio_vib_remove(struct platform_device *pdev)
{
	debugk(">>platform_set_drvdata(%p, NULL)\n", pdev);
	platform_set_drvdata(pdev, NULL);

	return 0;
}
static const struct of_device_id of_gpio_vib_match[] = {
	{ .compatible = "gpio-vib", },
	{},
};
static struct platform_driver gpio_vib_driver = {
	.probe		= gpio_vib_probe,
	.remove		= gpio_vib_remove,
	.driver		= {
		.name	= "gpio-vib",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(of_gpio_vib_match),
	},
};
module_platform_driver(gpio_vib_driver);
