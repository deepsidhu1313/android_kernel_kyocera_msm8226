...
static unsigned int kc_no_sdio_ports;
...
static int kc_gport_setup(struct usb_configuration *c)
{
	...
	pr_debug("%s: kc_no_tty_ports: %u kc_no_sdio_ports: %u"
		" kc_no_smd_ports: %u kc_no_hsic_sports: %u no_hsuart_ports: %u kc_nr_ports: %u\n",
			__func__, kc_no_tty_ports, kc_no_sdio_ports, kc_no_smd_ports,
			kc_no_hsic_sports, kc_no_hsuart_sports, kc_nr_ports);

	if (kc_no_tty_ports)
		ret = gserial_setup(c->cdev->gadget, kc_no_tty_ports);
	if (kc_no_smd_ports)
...
kc_gser_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	f->descriptors = usb_copy_descriptors(kc_gser_fs_function);
		...
		const char *port_name)
{
	...
	case USB_GADGET_XPORT_TTY:
		...
		break;
	case USB_GADGET_XPORT_SDIO:
		kc_gserial_ports[port_num].client_port_num = kc_no_sdio_ports;
		kc_no_sdio_ports++;
		break;
	case USB_GADGET_XPORT_SMD:
