...
#define kc_dbg(instance, format, arg...)            \
    if (phy_msm_usb_debug_log)                          \
        dev_info(instance , format , ## arg)

bool phy_msm_usb_debug_log = false;
module_param(phy_msm_usb_debug_log, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(phy_msm_usb_debug_log, "Enable KC Log Output");
...
static uint vbus_detect_count = 0;
module_param(vbus_detect_count, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(vbus_detect_count, "Current VBUS detect count");
...
bool is_vbus_active(void)
{
       ...
       return (motg == NULL) ? false :
               (test_bit(B_SESS_VLD, &motg->inputs)) ? true : false;
}

static const char *state_to_string(enum usb_chg_state state)
{
    switch (state) {
    case USB_CHG_STATE_UNDEFINED:       return "STATE_UNDEFINED";
    case USB_CHG_STATE_WAIT_FOR_DCD:    return "STATE_WAIT_FOR_DCD";
    case USB_CHG_STATE_DCD_DONE:        return "STATE_DCD_DONE";
    case USB_CHG_STATE_PRIMARY_DONE:    return "STATE_PRIMARY_DONE";
    case USB_CHG_STATE_SECONDARY_DONE:  return "STATE_SECONDARY_DONE";
    case USB_CHG_STATE_DETECTED:        return "STATE_DETECTED";
    default:            return "STATE_UNKNOWN";
    }
}

static const char *kc_otg_state_to_string(enum usb_otg_state state)
{
    switch (state) {
    ...
    default:            return "OTG_STATE_UNKNOWN";
    }
}
...
static bool msm_otg_connect_chg_pma_only(struct msm_otg *motg)
{
	if (atomic_read(&motg->usb_det) < 0) {
		dev_info(motg->phy.dev, "motg->usb_det = %d\n", atomic_read(&motg->usb_det));
		return false;
	}

	if (atomic_read(&motg->pma_det) < 0) {
		dev_info(motg->phy.dev, "motg->pma_det = %d\n", atomic_read(&motg->pma_det));
		return false;
	}

	/* USB is not connected & Power Matters Alliance(PMA) connected */
	if ((atomic_read(&motg->usb_det) == 0) && (atomic_read(&motg->pma_det) == 1))
		return true;
...
static bool msm_chg_check_dcd(struct msm_otg *motg)
{
	...
	default:
        ...
        kc_dbg(motg->phy.dev, "= USB DET I = : %s dcd = %s\n", __func__, ret?"true":"false");
	return ret;
}
...
static void msm_ta_detect_work(struct work_struct *w)
{
    ...
    if(motg->chg_type != USB_SDP_CHARGER) {
        pr_err("msm_ta_detect_work: don't access USB register - Not SDP_CHARGER\n");
        return;
    }
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	dev_dbg(phy->dev, "chg detection work\n");
        ...
        kc_dbg(motg->phy.dev, "= CHG DET I = : chg_state = %s\n",
            state_to_string(motg->chg_state));
	...
	case USB_CHG_STATE_WAIT_FOR_DCD:
		...
		tmout = motg->dcd_time >= MSM_CHG_DCD_TIMEOUT;
                kc_dbg(motg->phy.dev, "= CHG DET I = : is_dcd = %d, tmout = %d\n",
                    is_dcd, tmout);
		if (is_dcd || tmout) {
			if (is_dcd)
				dcd = true;
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
		...
		} else if (msm_otg_connect_chg_pma_only(motg)) {
			/* PMA charge */
			msm_chg_enable_secondary_det(motg);
			delay = MSM_CHG_SECONDARY_DET_TIME;
			motg->chg_state = USB_CHG_STATE_PRIMARY_DONE;
			...
			if (line_state) /* DP > VLGC or/and DM > VLGC */
				...
				motg->chg_type = USB_FLOATED_CHARGER;
			else if ((oem_chg_get_pma_det_status() == 1) && (oem_chg_get_usb_det_status() == 0)) {
				pr_info("Exceptional PMA charge");
				motg->chg_type = USB_DCP_CHARGER;
			} else
			...
			delay = 0;
                ...
                kc_dbg(motg->phy.dev, "= CHG DET I = : vout = %d, dcd = %d\n",
                    vout, dcd);
		break;
	case USB_CHG_STATE_PRIMARY_DONE:
		...
		motg->chg_state = USB_CHG_STATE_SECONDARY_DONE;
                kc_dbg(motg->phy.dev, "= CHG DET I = : vout = %d\n", vout);
		/* fall through */
	...
	case USB_CHG_STATE_DETECTED:
		...
		queue_work(system_nrt_wq, &motg->sm_work);
                kc_dbg(motg->phy.dev, "= CHG DET I = : chg_type = %s\n",
                    chg_to_string(motg->chg_type));
		return;
	default:
		return;
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_debug("%s work\n", otg_state_string(otg->phy->state));
        kc_dbg(motg->phy.dev, "= USB DET I = : otg_state = %s\n",	
            kc_otg_state_to_string(otg->phy->state));
	pr_info("msm_otg_sm_work phy->state=%d\n", otg->phy->state);
	pr_info("msm_otg_sm_work motg->chg_state=%d\n", motg->chg_state);
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!test_bit(B_SESS_VLD, &motg->inputs) &&
				test_bit(ID, &motg->inputs)) {
            kc_dbg(motg->phy.dev, "= USB DET I = : No device in %s\n", __func__);
			if (motg->online)
			    msm_otg_notify_charger(motg, 0);
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			pr_debug("b_sess_vld\n");
                        kc_dbg(motg->phy.dev, "= USB DET I = : vbus detect in %s charger type %s\n",
                            __func__, chg_to_string(motg->chg_type));
			switch (motg->chg_state) {
			case USB_CHG_STATE_UNDEFINED:
				vbus_detect_count++;
                                kc_dbg(motg->phy.dev, "= CHG DET I = : start charging detection work\n");
			...
			pr_debug("chg_work cancel");
                        kc_dbg(motg->phy.dev, "= CHG DET I = : canceling charging detection work\n");
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			switch_set_state(&sdev, 0xff);
			switch_set_state(&sdev, 0x00);
                        kc_dbg(motg->phy.dev, "= USB DET I = : vbus not detect in %s\n", __func__);
			motg->chg_state = USB_CHG_STATE_UNDEFINED;
			motg->chg_type = USB_INVALID_CHARGER;
			msm_otg_notify_charger(motg, 0);
...
static void msm_otg_set_vbus_state(int online)
{
	...
	if (online) {
		...
		atomic_set(&motg->usb_det, 0);
		atomic_set(&motg->pma_det, 0);
		clear_bit(B_SESS_VLD, &motg->inputs);
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		msm_otg_set_vbus_state(val->intval);
                kc_dbg(motg->phy.dev, "= CHG DET I = : vbus detect %s\n", __func__);
		break;
...
{
	struct msm_otg *motg = the_msm_otg;
	dev_info(motg->phy.dev, "change_charge_device( usb=%d ,pma=%d )\n"
			, usb_det, pma_det);

	if ((usb_det == atomic_read(&motg->usb_det))
	 && (pma_det == atomic_read(&motg->pma_det)))
	  {
		return;
	}

	/* USB and PMA conflict */
	if (usb_det && atomic_read(&motg->pma_det)) {
		dev_info(motg->phy.dev, "vbus is not cut\n");
		motg->chg_state = USB_CHG_STATE_UNDEFINED;
		motg->chg_type = USB_INVALID_CHARGER;
	}

	atomic_set(&motg->usb_det, usb_det);
	atomic_set(&motg->pma_det, pma_det);

	if ((usb_det == 1) && (pma_det == 1)) {
		/* vbus online processing for USB/AC */
		dev_dbg(motg->phy.dev, "USB/AC online processing\n");
	atomic_set(&motg->usb_det, usb_det);
		msm_otg_set_vbus_state(1);
	} else if ((usb_det == 1) && (pma_det == 0)) {
		/* vbus online processing for USB/AC */
		dev_dbg(motg->phy.dev, "USB/AC online processing\n");
		atomic_set(&motg->usb_det, usb_det);
		msm_otg_set_vbus_state(1);
	} else if ((usb_det == 0) && (pma_det == 1)) {
		/* vbus online processing for PMA */
		dev_dbg(motg->phy.dev, "PMA online processing\n");
		atomic_set(&motg->pma_det, pma_det);
		msm_otg_set_vbus_state(1);
	} else {
		/* vbus offline processing */
		dev_dbg(motg->phy.dev, "vbus offline processing\n");
		msm_otg_set_vbus_state(0);
	}
}
EXPORT_SYMBOL(msm_otg_change_charge_device);
