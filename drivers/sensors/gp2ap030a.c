...
#define REG_ADR_12	0x12
#define REG_ADR_13	0x13
#define REG_ADR_14	0x14
#define REG_ADR_15	0x15
#define REG_ADR_16	0x16
#define REG_ADR_17	0x17
#define REG_ADR_18	0x18
#define REG_ADR_MAX	0x19
...
struct gp2ap_data
{
	...
	struct delayed_work ps_get_data_dwork;
...
static u16 nv_proximity_sensor_near[GP2AP_COLOR_VARI] =
    {0x0202,0x0012,0x0012,0x0012,0x0012,0x0012,0x0012,0x0012,0x0012,0x0012};
static u16 nv_proximity_sensor_far[GP2AP_COLOR_VARI] =
    {0x01FC,0x000C,0x000C,0x000C,0x000C,0x000C,0x000C,0x000C,0x000C,0x000C};
...
static struct workqueue_struct *ps_get_data_wq;
...
static void gp2ap_reschedule_get_ps_data_work(struct gp2ap_data *data);
...
static void gp2ap_ps_get_data_work_handler(struct work_struct *work);
...
static int setThresholdByEnable ( struct i2c_client *client);
static int setThresholdByInterrupt ( struct i2c_client *client);
static int overWS2_HWID(void);


static int overWS2_HWID()
{
    int ret = 0;
    if (OEM_get_board() >= OEM_BOARD_WS2_TYPE) {
        ret = 1;
    }
    return ret;
}
...
static int gp2ap_get_ps_data(struct i2c_client *client, u16 *psdata)
{
	int ret = 0;
	u8 buf[2];

	PSALS_DBG_LOG(KERN_INFO,"[IN]\n");
	ret = gp2ap_i2c_read(client, REG_ADR_10, buf, 2);
	if (ret < 0)
	{
		goto exit;
	}
	*psdata = (buf[1] << 8 | buf[0]);
exit:
	PSALS_DBG_LOG(KERN_INFO,"[OUT] ret=%d\n",ret);
	return ret;
}
...
static int setThresholdByEnable ( struct i2c_client *client) {
	...
	ret = gp2ap_i2c_read(client, REG_ADR_0E, buf,4);
	if (ret < 0) {
		PSALS_LOG(KERN_ERR, "read VALUE error  =%d\n", ret);
		return ret;
	}
	irdata = (buf[1] << 8) | buf[0];
	count = (buf[3] << 8) | buf[2];
	PSALS_DBG_LOG(KERN_INFO,"read IR(%d), ProxValue(%d)",irdata,count);
	if (irdata < 8192) {
		if (count >= 12) {
			PSALS_LOG(KERN_INFO, "set treshold enable far(20),near(21),A(0)\n");
			data->proximity_count = 0;
			ret = gp2ap_set_ps_threshold(client,20,21);
		} else {
			data->proximity_count = count;
			PSALS_LOG(KERN_INFO, "set treshold enable far(%d),near(%d),A(%d)\n",count + 9, count + 11,count);
			ret = gp2ap_set_ps_threshold(client,data->proximity_count + 9, data->proximity_count + 11);
		}
	} else {
		PSALS_LOG(KERN_INFO, "set treshold enable far(20),near(21),A(0)\n");
		data->proximity_count = 0;
...
static int setThresholdByInterrupt ( struct i2c_client *client) {
	...
	ret = gp2ap_i2c_read(client, REG_ADR_0E, buf,4);
	if (ret < 0) {
		PSALS_LOG(KERN_ERR, "read IR_VALUE error  =%d\n", ret);
		return ret;
	}
	irdata = (buf[1] << 8) | buf[0];
	count = (buf[3] << 8) | buf[2];
	...
	if (irdata < 8192) {
		...
		PSALS_LOG(KERN_INFO, "read PROXIMITY_INT count(%d) A(%d)\n",count,data->proximity_count);
		if (count < data->proximity_count) {
			PSALS_DBG_LOG(KERN_INFO, "set treshold int far(%d),near(%d)\n",count + 9, count + 11);
			ret = gp2ap_set_ps_threshold(client,count + 9, count + 11);
		}
	}
	return ret;
}
...
(struct i2c_client *client, u32 sensor_kind, enum op_flag onoff)
{
	...
	if (pre_op_mode != data->op_mode)
	{
		...
		buf[0] = 0x4E;
		ret = gp2ap_i2c_write(client, REG_ADR_02, buf, 1);
		if (ret < 0)
		{
			PSALS_LOG(KERN_ERR, "gp2ap_i2c_write error. ret=%d\n", ret);
		}
		buf[0] = 0x38;
		ret = gp2ap_i2c_write(client, REG_ADR_03, buf, 1);
		if (ret < 0)
		{
			PSALS_LOG(KERN_ERR, "gp2ap_i2c_write error. ret=%d\n", ret);
		}
		buf[0] = 0x00;
		ret = gp2ap_i2c_write(client, REG_ADR_16, buf, 1);
		if (ret < 0)
		{
			PSALS_LOG(KERN_ERR, "gp2ap_i2c_write error. ret=%d\n", ret);
		}
		buf[0] = 0x00;
		ret = gp2ap_i2c_write(client, REG_ADR_18, buf, 1);
		if (ret < 0)
		{
			PSALS_LOG(KERN_ERR, "gp2ap_i2c_write error. ret=%d\n", ret);
		}
...
(struct i2c_client *client, enum sensor_enable enable)
{
	...
	if (enable == 1)
	{
		...
		if (overWS2_HWID()) {
			ret = setThresholdByEnable(client_gp2ap);
		}
		if (ret < 0)
		{
			PSALS_LOG(KERN_ERR, "gp2ap_set_threshold_by_enable error. ret=%d\n", ret);
			goto exit;
		}
...
static void gp2ap_reschedule_get_ps_data_work(struct gp2ap_data *data)
{
	PSALS_LOG(KERN_NOTICE, "[IN]\n");

	queue_delayed_work(ps_get_data_wq, &data->ps_get_data_dwork, msecs_to_jiffies(0));
...
static void gp2ap_ps_get_data_work_handler(struct work_struct *work)
{
	struct gp2ap_data *data = container_of(work, struct gp2ap_data, ps_get_data_dwork.work);
	int ret = 0;
    u16 psdata[1] = {0x0000};
	int colvar = nv_prox_photo_colvar[0];
	PSALS_LOG(KERN_NOTICE, "[IN]\n");
	mutex_lock(&data->psals_mutex);
	msleep(30);
	ret = gp2ap_get_ps_data(data->client,psdata);
	if (ret == 0)
	{
		if (!overWS2_HWID()) {
			if (psdata[0] <= nv_proximity_sensor_far[colvar])
			{
				input_report_abs(data->input_dev_ps,ABS_DISTANCE_REPORT,ABS_DUMMY_VAL);
				input_report_abs(data->input_dev_ps,ABS_DISTANCE_REPORT,1);
				input_sync(data->input_dev_ps);
				PSALS_LOG(KERN_NOTICE,"Enable ABS_DISTANCE_REPORT(detect:far) raw_value = 0x%04X \n",psdata[0]);
			}
		} else {
			if (psdata[0] <= 20)
			{
	...
	{
		PSALS_LOG(KERN_ERR,"can not get ps data");
	}
	PSALS_DBG_LOG(KERN_INFO, "[OUT]\n");
	mutex_unlock(&data->psals_mutex);
}
...
static void gp2ap_work_handler(struct work_struct *work)
{
	...
	ret = gp2ap_get_ps_data(client,psData);
	if(ret == 0)
	{
		PSALS_LOG(KERN_NOTICE,"Detect PS_DATA = 0x%04x \n",psData[0]);
	}
	...
	if (data->ps_detection == 0)
	{
		...
		if (overWS2_HWID()) {
			setThresholdByInterrupt(client);
		}
	}
	else
	{
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1)
	{
		if (data->enable_ps_sensor <= 0)
		{
			...
			gp2ap_reschedule_get_ps_data_work(data);
		}
		else
		{
			data->enable_ps_sensor++;
...
static DEVICE_ATTR(ps_enable, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(als_enable, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(als_poll_delay, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(ps_detection, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
				   ...
				   const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&data->ps_get_data_dwork,gp2ap_ps_get_data_work_handler);
...
static int __init gp2ap_init(void)
{
	...
	ps_get_data_wq = create_singlethread_workqueue("ps_get_data_wq");
	if (!ps_get_data_wq)
	{
		PSALS_LOG(KERN_ERR, "can't create queue : ps_get_data_wq \n");
		goto REGIST_ERR3;
	}
...
REGIST_ERR4:
	if (ps_get_data_wq != NULL)
	{
		flush_workqueue(ps_get_data_wq);
		destroy_workqueue(ps_get_data_wq);
		ps_polling_wq = NULL;
	}
