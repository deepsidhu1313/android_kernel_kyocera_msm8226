...
static struct qpnp_pon* the_pon;
void oem_notify_power_key_event(int code, int value)
{
	struct qpnp_pon* pon = the_pon;

	if (!pon) {
		return;
	}

	input_report_key(pon->pon_input, code, 1);
	input_report_key(pon->pon_input, code, 0);
