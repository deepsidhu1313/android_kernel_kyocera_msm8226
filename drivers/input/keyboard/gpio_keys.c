...
static struct gpio_keys_chattering_state gpio_keys_chattering_data[] = {
	{
		KEY_SEND, false
	},
	{
			    ...
			    struct gpio_keys_platform_data *pdata)
{
	...
	while ((pp = of_get_next_child(node, pp))) {
		...
		if (of_property_read_u32(pp, "on_chattering_num",
					&buttons[i].on_chattering_num)){
			buttons[i].on_chattering_num = 1;
		}
		buttons[i].on_cnt = buttons[i].on_chattering_num;
        
		KEY_LOG_PRINT("code:%d reg on_chattering_num:%d on_cnt:%d \n", buttons[i].code, buttons[i].on_chattering_num, buttons[i].on_cnt);

		if (of_property_read_u32(pp, "off_chattering_num",
					&buttons[i].off_chattering_num)){
			buttons[i].off_chattering_num = 0;
		}
		buttons[i].off_cnt = buttons[i].off_chattering_num;
        
		KEY_LOG_PRINT("code:%d reg off_chattering_num:%d off_cnt:%d \n", buttons[i].code, buttons[i].off_chattering_num, buttons[i].off_cnt);
...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&gpio_wake_lock,WAKE_LOCK_SUSPEND,"gpio_keys_wake_lock");
