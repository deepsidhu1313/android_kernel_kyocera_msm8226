...
#define CY_CFG_FILE_NAME	"cyttsp5_cfg.bin"
#define CY_DEVPARAM_FW_FILE_NAME  "cyttsp5_devparam_fw.bin"
#define CY_DEVPARAM_CFG_FILE_NAME "cyttsp5_devparam_cfg.bin"
...
#define CY_WATCHDOG_TIMEOUT			5000
...
#define KC_TOUCH_EVENT_RESTART          200
#define KC_TOUCH_EVENT_STOP             201

#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
#define OEM_STATE_RESUME_BIT            (1<<0)
#define OEM_STATE_SWITCH_BIT            (1<<1)
#define OEM_STATE_LOADING_BIT           (1<<2)
#define OEM_STATE_FACTORY_BIT           (1<<3)
#define OEM_STATE_STARTUP_BIT           (1<<4)
...
#define CY_MAX_AREA_SIZE		20
#define CY_MAX_PRESSURE_SIZE	250

/* FW RAM parameters */
#define CY_RAM_ID_TOUCHMODE_ENABLED	0xD0
#define CY_RAM_ID_PROXIMITY_ENABLE	0x20

#define REVISION_OF_GLOVE	800821

enum cyttsp5_touchmode_enabled_mode {
	CY_TME_NULL = 0,
	CY_TME_FINGER_ONLY = 1,
	CY_TME_FINGER_AND_GLOVE = 3,
};
...
struct cyttsp5_core_commands {
	...
	int (*request_set_touchmode_enabled)(struct device *dev, u8 mode);
	int (*request_set_state_loading)(struct device *dev, bool set);
	int (*request_loader_finished)(struct device *dev);
...
#define NEED_SUSPEND_NOTIFIER \
	...
	&& defined(CONFIG_PM_SLEEP) && defined(CONFIG_PM_RUNTIME) && defined(CY_USE_PM))

struct cyttsp5_touch_info {
	int x;
	int y;
	int wx;
	int wy;
...
#define CYTTSP5_PREALLOCATED_CMD_BUFFER 32
	...
	struct regulator *sensor_avdd;
	struct pinctrl *ts_pinctrl;
	struct pinctrl_state *gpio_state_active;
	struct pinctrl_state *gpio_state_suspend;
	struct kc_ts_data ts;
	struct cyttsp5_touch_info touch_info[KC_TS_MAX_FINGER];
	int touch_cnt;
