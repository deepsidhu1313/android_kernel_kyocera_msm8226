...
MODULE_FIRMWARE(CY_CFG_FILE_NAME);
...
static int cyttsp5_suspend(struct cyttsp5_core_data *cd);
static int cyttsp5_resume(struct cyttsp5_core_data *cd);
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
static int cyttsp5_change_state(struct device *dev, int state);
...
static int _cyttsp5_request_set_touchmode_enabled(struct device *dev, u8 mode)
{
	...
	if(cd->touchmode_enabled == CY_TME_NULL){
		dev_info(cd->dev, "%s: skip for old revctrl\n",__func__);
		return 0;
	}

	cd->touchmode_enabled = mode;
...
static void cyttsp5_touchmode_work(struct work_struct *work)
{
	...
	if(cd->force_finger){
		dev_info(cd->dev, "%s: force finger\n",__func__);
		cyttsp5_set_touchmode_enabled(cd, CY_TME_FINGER_ONLY);
	}
	else{
		cyttsp5_set_touchmode_enabled(cd, cd->touchmode_enabled);
	}
}
...
static int _cyttsp5_request_set_state_loading(struct device *dev, bool set)
{
	...
	if(set){
		rc = cyttsp5_change_state(cd->dev, cd->oem_state | OEM_STATE_LOADING_BIT);
	}
	else{
		rc = cyttsp5_change_state(cd->dev, cd->oem_state & ~OEM_STATE_LOADING_BIT);
	}
...
static void cyttsp5_watchdog_work(struct work_struct *work)
{
	...
	if (!(state & OEM_STATE_RESUME_BIT)) {
		dev_info(cd->dev, "%s: skip because lcd off\n", __func__);
		rc = 0;
		goto queue_startup;
	}
	...
	mutex_unlock(&cd->loader_lock);
	if (!loader_finished) {
		dev_info(cd->dev, "%s: skip becaouse loader is not finished\n", __func__);
		rc = 0;
		goto queue_startup;
	}
...
//	cyttsp5_hid_output_bl_get_panel_id_(cd, &cd->panel_id);
	...
	if(cd->sysinfo.cydata.revctrl>=REVISION_OF_GLOVE){
		cd->touchmode_enabled = CY_TME_FINGER_AND_GLOVE;
	}
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	mutex_lock(&cd->change_lock);
	cyttsp5_change_state(cd->dev, cd->oem_state | OEM_STATE_STARTUP_BIT);
	mutex_unlock(&cd->change_lock);
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
...
static int cyttsp5_change_state(struct device *dev, int state)
{
	...
	if(0 == old_state && 0 < cd->oem_state) {
		enable_irq_wake(cd->irq);
		rc = cyttsp5_resume(cd);
		if (rc)
	...
	else 	if(0 < old_state && 0 == cd->oem_state) {
		disable_irq_wake(cd->irq);
		rc = cyttsp5_suspend(cd);
		if (rc)
	     ...
	     && 0 == (cd->oem_state & OEM_STATE_SWITCH_BIT)) {
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
		mutex_lock(&cd->change_lock);
		rc = cyttsp5_change_state(cd->dev, cd->oem_state & ~OEM_STATE_FACTORY_BIT);
		mutex_unlock(&cd->change_lock);
#else
		rc = cyttsp5_suspend(cd);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
			mutex_lock(&cd->change_lock);
			cyttsp5_change_state(cd->dev, cd->oem_state | OEM_STATE_RESUME_BIT);
			mutex_unlock(&cd->change_lock);
#else
			if(cyttsp5_resume(cd))
...
static int cyttsp5_get_system_info_ioctl(struct kc_ts_data *ts, u8 *dp)
{
	...
	mutex_lock(&cd->state_lock);
	if(cd->oem_state & OEM_STATE_LOADING_BIT){
...
static int cyttsp5_sensor_switch_ioctl(struct kc_ts_data *ts, int value)
{
	...
	if (value) {
		...
		mutex_lock(&cd->change_lock);
		cyttsp5_change_state(cd->dev, cd->oem_state | OEM_STATE_SWITCH_BIT);
		mutex_unlock(&cd->change_lock);
	}
	else {
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->touchmode_enabled = CY_TME_NULL;
	cd->force_finger = false;
	cd->loader_finished = false;
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	cd->oem_state = OEM_STATE_RESUME_BIT;
#endif
