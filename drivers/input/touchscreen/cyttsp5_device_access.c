		...
		struct device_attribute *attr, char *buf)
{
	...
	if(dad->manufacturing_id=='s' || dad->manufacturing_id=='o'){
		...
		dad->get_idac_data_id = 1;
		rc = cyttsp5_get_data_structure_cmd_(dev, 0, CY_MAX_PRBUF_SIZE - offset,
				dad->get_idac_data_id, &cmd_status, &data_format,
				&act_length, &dad->ic_buf[offset]);
		if (rc < 0) {
			dev_err(dev, "%s: Error on get data structure r=%d\n",
		...
		dad->ic_buf[4] = cmd_status;

		dad->get_idac_data_id = 0;
		rc = cyttsp5_get_data_structure_cmd_(dev, 0, CY_MAX_PRBUF_SIZE - length,
				dad->get_idac_data_id, &cmd_status, &data_format,
				&act_length, &dad->ic_buf[length]);
		if (rc < 0) {
			dev_err(dev, "%s: Error on get data structure r=%d\n",
					__func__, rc);
			goto resume_scan;
		...
		length += act_length;
		dad->ic_buf[5] = cmd_status;
		dad->ic_buf[6] = LOW_BYTE(act_length);
		dad->ic_buf[7] = HI_BYTE(act_length);
	}
