...
struct cyttsp5_loader_data {
	...
	bool config_loading;
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_TTCONFIG_UPGRADE
	struct completion builtin_bin_cfg_complete;
	int builtin_bin_cfg_status;
#endif
};
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (fw_ver_new > fw_ver_img) {
		dev_notice(dev, "%s: Image is newer, will upgrade\n",
				__func__);
		return 1;
	}
...
static void cyttsp5_calibrate_idacs(struct work_struct *calibration_work)
{
	...
	dev_info(dev, "%s: Calibration Done\n", __func__);
		...
		int fw_size)
{
	...
	rc = cmd->request_set_state_loading(dev, true);
	if (rc < 0)
		dev_err(dev, "%s: Set on state_loading failed rc=%d\n", __func__, rc);
...
exit:
	...
	rc = cmd->request_set_state_loading(dev, false);
	if (rc < 0)
		dev_err(dev, "%s: Set off state_loading failed rc=%d\n", __func__, rc);
		...
		struct device *dev)
{
	...
	while ((fw = *fws++)) {
		if (fw->panel_id == panel_id) {
			dev_info(dev, "%s: Found matching fw:%p with Panel ID: 0x%02X\n",
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
	reinit_completion(&ld->builtin_bin_fw_complete);
#else
...
static int upgrade_firmware_from_devparam(struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	int retval;
	...
	dev_info(dev, "%s: Filename: %s\n", __func__, CY_DEVPARAM_FW_FILE_NAME);
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			CY_DEVPARAM_FW_FILE_NAME, dev, GFP_KERNEL, dev,
			_cyttsp5_firmware_cont_builtin);
	if (retval < 0) {
		dev_err(dev, "%s: Fail request firmware class file load\n",
			__func__);
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_TTCONFIG_UPGRADE
static void _cyttsp5_ttconfig_cont_builtin(const struct firmware *fw,
		void *context)
{
	struct device *dev = context;
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	...
	ld->builtin_bin_cfg_status = -EINVAL;

	if (!fw) {
		dev_info(dev, "%s: No builtin ttconfig\n", __func__);
		goto _cyttsp5_ttconfig_cont_builtin_exit;
	}

	if (!fw->data || !fw->size) {
		dev_err(dev, "%s: Invalid builtin ttconfig\n", __func__);
		goto _cyttsp5_ttconfig_cont_builtin_exit;
	}

	dev_dbg(dev, "%s: Found ttconfig\n", __func__);
	ld->config_data = (u8 *)fw->data;
	ld->config_size = fw->size;
	...
	cfg_ver_new = (ld->config_data[24] << 8) + (ld->config_data[25]);
	...
	if (cfg_ver_new > ld->si->cydata.fw_ver_conf) {
		...
		__func__);
		rc = cyttsp5_upgrade_ttconfig(dev, start, length);
		ld->builtin_bin_cfg_status = 0;
	}

_cyttsp5_ttconfig_cont_builtin_exit:
	release_firmware(fw);
	ld->config_data = NULL;
	ld->config_size = 0;

	complete(&ld->builtin_bin_cfg_complete);
}

static int upgrade_ttconfig_from_builtin(struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	int retval;

	dev_vdbg(dev, "%s: Enabling ttconfig class loader built-in\n",
		__func__);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
	reinit_completion(&ld->builtin_bin_cfg_complete);
#else
	INIT_COMPLETION(ld->builtin_bin_cfg_complete);
#endif

	dev_info(dev, "%s: Filename: %s\n", __func__, CY_CFG_FILE_NAME);
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			CY_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
			_cyttsp5_ttconfig_cont_builtin);
	if (retval < 0) {
	...
	wait_for_completion(&ld->builtin_bin_cfg_complete);

	retval = ld->builtin_bin_cfg_status;

exit:

	return retval;
}
#endif /* CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_TTCONFIG_UPGRADE */

#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVPARAM_UPGRADE
static int upgrade_ttconfig_from_devparam(struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	int retval;

	dev_vdbg(dev, "%s: Enabling ttconfig class loader devparam\n",
		__func__);
	...
	dev_info(dev, "%s: Filename: %s\n", __func__, CY_DEVPARAM_CFG_FILE_NAME);
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			CY_DEVPARAM_CFG_FILE_NAME, dev, GFP_KERNEL, dev,
			_cyttsp5_ttconfig_cont_builtin);
	if (retval < 0) {
		dev_err(dev, "%s: Fail request ttconfig class file load\n",
			__func__);
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVPARAM_UPGRADE
	if (!upgrade_ttconfig_from_devparam(dev))
		dev_dbg(dev, "%s: upgrade ttconfig of devparam is completed\n",__func__);
#endif

#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_TTCONFIG_UPGRADE
	if (!upgrade_ttconfig_from_builtin(dev))
		goto exit;
#endif

exit:
	cmd->request_loader_finished(dev);
}
