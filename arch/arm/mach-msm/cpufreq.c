...
#define LOWTEMPMODE_FREQ (787200)
static int lowtempmode = 0;
static unsigned int msm_cpu_freq_min = CONFIG_MSM_CPU_FREQ_MIN;
static unsigned int msm_cpu_freq_max = CONFIG_MSM_CPU_FREQ_MAX;
#endif
...
#ifdef CONFIG_MSM_CPU_FREQ_SET_MIN_MAX
		policy->cpuinfo.min_freq = msm_cpu_freq_min;
		policy->cpuinfo.max_freq = msm_cpu_freq_max;
#endif
	}
#ifdef CONFIG_MSM_CPU_FREQ_SET_MIN_MAX
	policy->min = msm_cpu_freq_min;
	policy->max = msm_cpu_freq_max;
#endif
...
#ifdef CONFIG_MSM_CPU_FREQ_SET_MIN_MAX
static int __init setup_lowtempmode(char *str)
{
	get_option(&str, &lowtempmode);

	if (lowtempmode == 1) {
		msm_cpu_freq_max = LOWTEMPMODE_FREQ;
	} else if (lowtempmode == 2) {
		msm_cpu_freq_max = LOWTEMPMODE_FREQ;
	}

	return 0;
}
early_param("lowtempmode", setup_lowtempmode);
#endif
