...
typedef struct {
    ...
    unsigned int ddr_data_info[ ( KCC_SMEM_DDR_DATA_INFO_SIZE + 4 ) / sizeof( unsigned int ) ];
    unsigned int kc_wm_uuid[ ( KCC_SMEM_KC_WM_UUID_SIZE + 4 ) / sizeof( unsigned int ) ];
    ...
    unsigned int imei[ ( KCC_SMEM_IMEI_SIZE + 4 ) / sizeof( unsigned int ) ];
    unsigned int imei_check[ ( KCC_SMEM_IMEI_CHECK_SIZE + 4 ) / sizeof( unsigned int ) ];
    unsigned int is_off_chg[ ( KCC_SMEM_IS_OFF_CHG_SIZE + 4 ) / sizeof( unsigned int ) ];
} type_kcc_smem;
...
void *kc_smem_alloc(unsigned smem_type, unsigned buf_size)
{
	...
	switch (smem_type)
	{
        ...
        case SMEM_IMEI_CHECK:
            if( buf_size <= KCC_SMEM_IMEI_CHECK_SIZE )
            {
                return( ( void * )&kc_smem_addr->imei_check[ 0 ] );
            }
            break;

        case SMEM_IS_OFF_CHG:
            if( buf_size <= KCC_SMEM_IS_OFF_CHG_SIZE )
            {
                return( ( void * )&kc_smem_addr->is_off_chg[ 0 ] );
            }
            break;
...
#define KERNEL_LOG_START_OFFSET	(0x000001F0)
#define KERNEL_LOG_BUFF_ADDR	(MSM_UNINIT_RAM_BASE + KERNEL_LOG_START_OFFSET)
#define KERNEL_LOG_BUFF_SIZE	(0x00080000)

extern void switch_printk_buffer( unsigned char *dist_addr, unsigned long buff_size );
...
#endif /* #ifdef CONFIG_ANDROID_RAM_CONSOLE */
	switch_printk_buffer( KERNEL_LOG_BUFF_ADDR, KERNEL_LOG_BUFF_SIZE );
	msm8226_add_drivers();
}
...
{
	unsigned int* secboot_mode = (unsigned int*)kc_smem_alloc(SMEM_SECUREBOOT_FLAG, sizeof(secboot_mode));
...
EXPORT_SYMBOL(oem_is_recoverymode);
