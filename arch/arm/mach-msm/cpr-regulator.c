...
static int pvs = -1;
static int avs_svs = -1;
static int avs_nominal = -1;
static int avs_turbo = -1;
module_param(pvs, int, S_IRUGO | S_IWUSR);
module_param(avs_svs, int, S_IRUGO | S_IWUSR);
module_param(avs_nominal, int, S_IRUGO | S_IWUSR);
module_param(avs_turbo, int, S_IRUGO | S_IWUSR);
			       ...
			       struct cpr_regulator *cpr_vreg)
{
	...
	u32 avs_fuse[4], avs_fuse_sbit[3];
	...
	pvs = cpr_vreg->pvs_bin;

	rc = of_property_read_u32_array(of_node, "qcom,avs-fuse",
					avs_fuse, 4);
	if (rc < 0) {
		pr_err("avs-fuse missing: rc=%d\n", rc);
		return rc;
	}

	rc = of_property_read_u32_array(of_node, "qcom,avs-fuse-startbit",
					avs_fuse_sbit, 3);
	if (rc < 0) {
		pr_err("avs-fuse-startbit missing: rc=%d\n", rc);
		return rc;
	}

	efuse_bits = cpr_read_efuse_row(cpr_vreg, avs_fuse[0], avs_fuse[3]);
	avs_turbo = (efuse_bits >> avs_fuse_sbit[0]) & ((1 << avs_fuse[2]) - 1);
	avs_nominal = (efuse_bits >> avs_fuse_sbit[1]) & ((1 << avs_fuse[2]) - 1);
	avs_svs = (efuse_bits >> avs_fuse_sbit[2]) & ((1 << avs_fuse[2]) - 1);
